...
suicide_funnel %>%
  ggplot() +
  geom_histogram(aes(x = rate), binwidth = 0.001)
```
Based on the trend above, this is not normal distribution. However, it is still interesting to check the funnel plot distribution as a visual aid for detecting bias or systematic heterogeneity

#### 6.2 Funnel Plot

```{r}
plot_funnel <- function(period1, period2, type = "PR"){
...
funnel_plot(
  numerator = suicide_funnel$SN,
  denominator = suicide_funnel$DN,
  group = suicide_funnel$country,
  data_type = type,
  xrange = c(0, 75000000), 
  multiplier = 10,
  label_outliers = TRUE,
  title = paste0("Cumulative Suicide Rate by Cumulative Total Number of Deaths from ",period1," to ",period2),            
  x_label = "Cumulative Number of Deaths"
)
}
```
